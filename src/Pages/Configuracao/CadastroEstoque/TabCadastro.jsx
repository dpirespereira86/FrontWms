...
const TabCadastro = (props) => {
  ...
  const col = ['nome', 'capacidade', 'quantidade'];
  const title = ['Nome', 'Capacidade(KG)', 'Quantidade(KG)', ''];
  console.log(props);
  useEffect(() => {
    axios
      .get(base + 'v1/' + localStorage.getItem('empresa') + '/estoques', {
        headers: {
  ...
  return (
    <div>
      <SimpleTable
        ...
        setDados={setDados}
        setOpen={setOpen}
        end={base + 'v1/' + localStorage.getItem('empresa') + '/estoques/'}
      />
      <Modal
        open={open}
        onClose={handleClose}
